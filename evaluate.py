...
for model_name, ModelClass in models.items():
    ...
    model_results_dir = os.path.join("evaluation_results", model_name)
    ...
    for folder in stage_sensor_folders:
        ...
        # Перевірка, чи належить папка поточному типу моделі
        if not folder.startswith(os.path.join("models", model_name)):
            print(f"Skipping folder (wrong model type): {folder}")
            continue

        # Перевірка, чи є folder папкою
        if not os.path.isdir(folder):
            print(f"Skipping path (not a folder): {folder}")
            continue
        ...
        plt.legend()
        plt.savefig(os.path.join(folder, f"{model_filename}_plot.png")) # Змінено шлях до папки
        plt.close()
        print(f"    Saved plot to {os.path.join(folder, f'{model_filename}_plot.png')}")
        ...
        if ad is not None and ad_pred is not None:
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig(os.path.join(folder, f"{model_filename}_allan_deviation.png"))# Змінено шлях до папки
            plt.close()
            print(f"    Saved Allan Deviation plot to {os.path.join(folder, f'{model_filename}_allan_deviation.png')}")
        ...
        if histories is not None:
            try:
                ...
                epoch_metrics_filepath = os.path.join(folder, f"{model_filename}_epoch_metrics.csv")
